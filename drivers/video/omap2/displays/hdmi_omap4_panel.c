...
static int get_timings_index(void)
{
	...
	if (code == -1)	{
		code = 9;
		hdmi.code = 16; /* HDMI code 16 corresponds to 1920 * 1080 */
		hdmi.mode = 1;	/* HDMI mode 1 corresponds to HDMI 0 to DVI */
	}
	return code;
}
...
static void get_edid_timing_data(u8 *edid)
{
	...
	pr_info("no valid timing found , falling back to VGA\n");
...
static void hdmi_core_av_packet_config(u32 name,
		struct hdmi_core_packet_enable_repeat repeat_cfg)
{
...
int hdmi_lib_init(void)
{
	...
	omapdss_hdmi_base_address(&hdmi_start, &size);
	/* Base address taken from platform */
	hdmi.base_wp = ioremap(hdmi_start, size);
	if (!hdmi.base_wp) {
...
static int hdmi_panel_enable(struct omap_dss_device *dssdev)
{
	...
	pr_info("ENTER hdmi_panel_enable\n");
	...
	omapdss_hdmi_configure_dispc(dssdev);
